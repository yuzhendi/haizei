/*************************************************************************
	> File Name: double_arrar_trie.cpp
	> Author: yu zhen di
	> Mail: 1316215578@qq.com
	> Created Time: 2020年05月06日 星期三 09时06分52秒
 ************************************************************************/
...
int insert(Node *root, const char *str) {//用字典树来构造，实际上不需要用到树结构
    int cnt = 0;
    Node *p = root;
    for (int i = 0; str[i]; i++) {
...
int getBaseValue(Node *root, DATrie *tree) {
    ...
    do {
        ...
        for (int i = 0; i < BASE; i++) {
            ...
            flag = 0;//找到root第一个孩子，第一个数组中被占用的下标
            break;
        }
    } while (!flag);
    return base;
...
int buildDATrie(int ind, Node *root, DATrie *tree) {//
    ...
    for (int i = 0; i < BASE; i++) {
        if (root->next[i] == NULL) continue;
        tree->check[base + i] = ind;//孩子节点中存放父亲节点下标
    }
    for (int i = 0; i < BASE; i++) {
        if (root->next[i] == NULL) continue;
    ...
    if (root->flag) tree->check[ind] = -tree->check[ind];//独立成词
    return ans;
}
