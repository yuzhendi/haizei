...
void initfootball() {
   ...
   init_pair(11, COLOR_BLACK, COLOR_WHITE);
    ...
    Football_t = create_newwin(court.width + 4, court.heigth + 2, court.start.x - 2, court.start.y - 1) ;  //绘画一个球场边框              
    wrefresh(Football_t);
    Football = subwin(Football_t, court.width, court.heigth, court.start.x, court.start.y);
    box(Football_t, 0, 0);
    wrefresh(Football_t);
    ...
    wrefresh(Score);
    Write = create_newwin(24 + court.width, 5, court.start.x - 2 , court.start.y + court.heigth + 8);
    wrefresh(Write);
