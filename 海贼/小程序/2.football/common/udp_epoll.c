/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: Thu 04 Jun 2020 07:07:03 PM CST
 ************************************************************************/
...
int check_online(struct LogRequest *request) {
    ...
    for (int i = 0; i < MAX; i++) {
        if (team[i].online && !strcmp(team[i].name, request.name)) return 1;
    }*/
    for (int i = 0; i < MAX; i++) {
...
int udp_accept(int epollfd, int fd, struct User *user) {
    ...
    if (check_online(&request)) {
        response.type = 1;
        strcpy(response.msg, "You are already exict the game!\n");
        sendto(fd, (void *)&response, sizeof(response), 0, (struct sockaddr *)&client, len);
        return -1;
    }
    response.type = 0;
